...
class FamilySafetyDataUpdateCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        if self.api is None:
            ...
            except UpdateFailed as _setup_err:
                if not self._is_stale_roster_error(str(_setup_err).lower()):
                    raise
                _LOGGER.warning(
                    "Family Safety: mobile API setup failed — a stale Entra ID / MDM "
                    "device is still enrolled in the family roster and Microsoft cannot "
                    "resolve it. The integration will run in degraded mode (web API "
                    "only). To restore full functionality remove the old device at "
                    "https://account.microsoft.com/devices or ask an admin to "
                    "de-register it from Microsoft Endpoint Manager / Intune."
                )
        ...
        try:
            ...
            for account in (self.api.accounts if self.api is not None else []):
